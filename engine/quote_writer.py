...
STATIC_MAKE_LIST = [
    ...
    {"item": "PRESSURE SWITCH",               "make": "MADAS / SWITZER"},
    {"item": "BALL VALVE",                    "make": "AUDCO/ L&T/ LEADER"},
    {"item": "PNEUMATIC CONTROL VALVE",       "make": "DEMBLA/AIRA"},
    {"item": "BUTTERFLY VALVE",               "make": "AUDCO/ L&T/ LEADER"},
    {"item": "ROTARY JOINT",                  "make": "ENCON/KRATOS"},
    ...
    {"item": "DPT",       "make": "HONEYWELL"},
    {"item": "GEARED MOTOR MECHANISM",       "make": "POWERTEK"},

    {"item": "BURNER",                        "make": "ENCON"},
    {"item": "PILOT BURNER",                  "make": "ENCON"},
    {"item": "BLOWER",                        "make": "ENCON"},
    {"item": "BEARING",                       "make": "FAG/SKF"},
    {"item": "UV SENSOR",                     "make": "ENCON"},
    {"item": "IGNITION TRANSFORMER",          "make": "COFI/DANFOSS"},
    {"item": "SEQUENCE CONTROLLER",           "make": "ENCON"},
    {"item": "MOTOR",                         "make": "ABB/CGL/BB"},
...
# the pumping unit) — these get a plain Pumping Unit. Anything else oil-based
# gets the heater (SKO, HDO, CFO, FO etc.). FO was previously pumping-only but
# now routes through the HPU selector so the in-unit heater is included.
PUMPING_UNIT_ONLY_FUEL_NAMES = {"HSD", "LDO", "LSHS"}
...
def _pumping_unit_block(fuel_name: str, is_oil: bool, is_dual: bool,
    ...
    if pumping_only:
        ...
        intro = (f"To supply fuel oil to the above burner at requisite "
                 ...
                 f"heating and pumping unit type {variant_label} "
...
def _cv_bullets_for_offer(control_mode: str, auto_control_type: str,
    ...
    """Build the Temperature Control System control-valve bullets per spec:
      ...
      - All other modes   : air + fuel CV (gas/oil/both for dual oil+gas)
...
def _temp_control_items_for_mode(control_mode: str, auto_control_type: str,
    ...
    if cm == "manual":
        ...
        if is_dual_with_oil:
            ...
            # fuel side. Ratio controllers are NOT listed under Temperature
            # Control System -- they're rendered separately on the line side.
            items = [
...
def generate_quote_docx(quote_data: dict, output_path: str,
    ...
    if customer.get("standalone_pumping"):
        ...
        if customer.get("force_pumping_only"):
            ...
            _pitems = [
                _pump_bullet,
                "1 No. Duplex type coarse filter for the cold oil side respectively.",
                "1 No. Pressure regulating valve.",
